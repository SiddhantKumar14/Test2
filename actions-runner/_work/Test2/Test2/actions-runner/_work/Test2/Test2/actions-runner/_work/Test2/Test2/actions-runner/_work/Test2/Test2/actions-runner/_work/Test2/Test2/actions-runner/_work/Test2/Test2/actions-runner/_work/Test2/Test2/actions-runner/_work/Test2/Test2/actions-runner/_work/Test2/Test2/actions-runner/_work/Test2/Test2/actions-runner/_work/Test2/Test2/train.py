...
def train(hyp, opt, device, tb_writer=None):
    ...
    with mlflow.start_run() as run:
        mlflow.pytorch.autolog(
            log_every_n_epoch=1, log_models=False, disable=True, exclusive=False
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--weights",
        type=str,
        default="mlflow_yolo_weights.pt",
        help="initial weights path",
    )
    parser.add_argument("--cfg", type=str, default="", help="model.yaml path")
    parser.add_argument(
        "--data", type=str, default="data_yml/licence_data.yaml", help="data.yaml path"
    )
    parser.add_argument(
        "--hyp",
        type=str,
        default="data_yml/hyp.scratch.yaml",
